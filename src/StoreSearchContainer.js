...
export default class StoresSearchContainer extends Component{
    ...
    render(){
        var storeList = this.state.stores.filter(store => store.name.toUpperCase().includes(this.state.filter.toUpperCase()) || store.address.toUpperCase().includes(this.state.filter.toUpperCase())).map(store => <Link className="text-center" to={`stores/${store.id}`} key={store.id}><div className="storeResult" key={store.id}><div style={{fontSize: '22px', }}>{store.name}</div><div style={{fontSize: '15px'}}>{store.address}</div></div></Link>)
        return(
          <div>
            <Switch>
